...
def x2p_torch(X, tol=1e-5, perplexity=30.0, device=None):
    """
...
def pca_torch(X, no_dims=50, device=None):
...
def tsne(X, no_dims=2, initial_dims=50, perplexity=30.0, device=None):
    """
    ...
    X = pca_torch(X, initial_dims, device)
    (n, d) = X.shape
    max_iter = 1000
    ...
    # Compute P-values
    P = x2p_torch(X, 1e-5, perplexity, device)
    P = P + P.t()
    P = P / torch.sum(P)
