...
class DyTox_prompt(DyTox):

    def __init__(self, transformer, nb_classes, individual_classifier='', head_div=False,
                 head_div_mode=['tr', 'ft'], joint_tokens=False):
        ...
        self.sabs = transformer.blocks[:5]
        ...
        self.instance_prompt = nn.Linear(384, 10, bias=False).cuda()
    ...
    def forward_features(self, x):
        ...
        x = x + self.pos_embed
        if len(self.nb_classes_per_task) == 1:
            x = self.pos_drop(x)
        else:
            instance_tokens = self.instance_prompt.weight.to(x.device) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype,device=x.device)
...
from models.convit import ClassAttention, Block
...
class DyTox_ptconvit(DyTox):
    def __init__(
        ...
        self.pos_drop = transformer.pos_drop
        self.cls_token = transformer.cls_token
        self.sabs = transformer.blocks[:8]
